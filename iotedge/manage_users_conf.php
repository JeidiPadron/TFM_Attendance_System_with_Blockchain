            ...
            if ($row['username'] == "None") {
                ...
                if (!empty($Uname) && !empty($Number)) {
                    if (!empty($_POST['gender'])) {
                        ...
                        $sql = "SELECT SocialSecurityId FROM users WHERE SocialSecurityId=? AND device_uid=?";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            echo "SQL_Error";
                            exit();
                            ...
                            if (!$row = mysqli_fetch_assoc($resultl)) {
                                ...
                                $sql="UPDATE users SET username=?, SocialSecurityId=?, gender=?, user_date=CURDATE() WHERE fingerprint_select=1 AND device_uid=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
            ...
            if ($row = mysqli_fetch_assoc($resultl)) {
                ...
                if ($row['add_fingerid'] == 1) {
                    ...
                    if (empty($Uname) && empty($Number)) {
                        ...
                        //check if there any user had already the Serial Number
                        $sql = "SELECT SocialSecurityId FROM users WHERE SocialSecurityId=? AND fingerprint_select=0";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            echo "SQL_Error";
                            exit();
                            ...
                            if (!$row = mysqli_fetch_assoc($resultl)) {                 
                                if (!empty($Uname)) {
                                    ...
                                    $sql="UPDATE users SET username=?, SocialSecurityId=?, gender=? WHERE fingerprint_select=1 AND device_uid=?";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        echo "SQL_Error_select_Fingerprint";
                                        exit();
